...
/// Helper to invoke special function objects (composed, negated, flipped, partial)
pub(crate) fn invoke_special_fn(func: &Object, args: Vec<Object>, env: EnvRef) -> Option<Object> {
    if let Object::Object(map) = func {
        // Handle composed functions
        if map.get("__is_composed__") == Some(&Object::Boolean(true)) {
            if let (Some(f), Some(g)) = (map.get("__compose_f__"), map.get("__compose_g__")) {
                let g_result = apply_function_with_this(g.clone(), args, None, env.clone());
                if matches!(g_result, Object::Error(_)) {
                    return Some(g_result);
                }
                return Some(apply_function_with_this(f.clone(), vec![g_result], None, env));
            }
        }
        ...
        // Handle flipped functions
        if map.get("__is_flipped__") == Some(&Object::Boolean(true)) {
            if let Some(func) = map.get("__flipped_fn__") {
                let mut flipped_args = args;
                if flipped_args.len() >= 2 {
                    flipped_args.swap(0, 1);
                }
                return Some(apply_function_with_this(func.clone(), flipped_args, None, env));
            }
        }

        // Handle partial application
        if map.get("__is_partial__") == Some(&Object::Boolean(true)) {
            if let (Some(func), Some(Object::Array(bound))) =
                (map.get("__partial_fn__"), map.get("__partial_args__"))
            {
                let mut all_args = bound.clone();
                all_args.extend(args);
                return Some(apply_function_with_this(func.clone(), all_args, None, env));
            }
        }
    }
    None
